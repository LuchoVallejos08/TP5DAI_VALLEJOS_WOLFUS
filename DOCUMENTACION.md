...
- **Caso de error 400:** `GET /api/evolution/abc` → 400, `{ "error": "El id de la cadena de evolución debe ser numérico" }`.

Todas las pruebas se verificaron primero por línea de comandos (`curl`) contra el servidor local antes de probarlas desde Swagger UI, confirmando que el código de estado y el cuerpo de la respuesta coinciden con lo documentado.
...
_(Agregar capturas de pantalla: vista principal de `/api-docs`, cada endpoint expandido, una prueba "Try it out" con su respuesta 200 y una con error 404/400)_
